...
class IndexController extends Controller {
    public function __construct(){
        ...
        if($adminId == 1)
            $sql = 'SELECT COUNT(*) has FROM dev_privilege WHERE '.$where;
        else 
            $sql = 'SELECT COUNT(a.role_id) has
              FROM dev_role_privilege a
               LEFT JOIN dev_privilege b ON a.pri_id=b.id
               LEFT JOIN dev_admin_role c ON a.role_id=c.role_id
    ...
    public function welcome(){
        ...
        $server = $_SERVER;
        $server['time'] = date('Y-m-d H:i:s',$server['REQUEST_TIME']);
